...
6. **功能模块**：4个模块+排盘可视化，每个含输入示例
7. **工作流**：Step 0（知识准备+按需回源验证）→ Step 1（收集输入）→ Step 2（排盘）→ Step 2.5（验前事校验）→ Step 3（格局识别）→ Step 4（生成报告MD+HTML）→ Step 5（继续提问，正反双审）→ Step 6（全面核查+自检清单+验证反馈闭环）
8. **参考索引**：18个references按需加载+8个脚本工具
9. **常见陷阱**：25条+正反对比示例，含内容丰富度分级制要求（重点宫7子节/普通宫5子节/轻量宫3子节、宫干飞四化48条、内容以示例为标准、差异化指引防公式化）
10. **知识补充**：网页搜索策略（仓库优先、冲突以仓库为准）、排盘失败降级方案、来源标注决策树
