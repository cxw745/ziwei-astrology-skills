...
10. **禁止滥用"不是…而是…"**：这个句式容易制造虚假对比和错误归因。只在确实存在明确对比关系时使用，且对比双方必须逻辑对等。大量"不是A而是B"的堆砌会让解读显得武断且缺乏因果逻辑
8. **首次输出完整报告**：用户第一次提供信息时输出完整命盘报告，内容丰富度必须达到示例水平（1000行以上），不可省略任何章节或子节
9. **来源标注**：每条事实性内容必须标注来源文件，格式 `[来源: 仓库名/文件路径]`，详见 `references/source-repos.md`
10. **盲审原则**：分析阶段只看排盘数据，不看用户背景，详见下方"盲审原则8条"
11. **文件优先输出**：所有完整内容必须写入文件，对话中只输出简要摘要+文件路径。禁止在对话中输出完整报告正文。详见下方"文件优先输出策略"
