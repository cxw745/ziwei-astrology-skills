...
4. **Q&A追问也必须写文件**：用户自由追问的回答，如果超过20行，必须追加写入该命盘的 `问答记录.md` 文件，对话中只输出简要回答。`\question` 指令则是AI向用户提问→用户回答→记录完整Q&A
...
```
🤔 {AI根据命盘提出的深度问题，1-2行，聚焦命盘中有故事性的要点}

（等用户回答后，再基于排盘数据进行分析，完整Q&A写入问答记录.md）
```
...
**⚠️ 文件优先**：如果回答超过20行，必须追加写入 `ziwei-output/{日期_HHmm}_{出生信息}/问答记录.md`，对话中只输出简要回答（见"文件优先输出策略"）。`\question` 指令执行AI向用户提问→用户回答→完整记录Q&A的三步流程。
