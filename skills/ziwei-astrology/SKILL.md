...
4. **Q&A记录管理**：用户自由追问的回答，如果超过20行，必须追加写入该命盘的 `问答记录.md` 文件，对话中只输出简要回答。`\question` 指令特殊规则：提问阶段纯对话输出（不写文件），用户回答后，再将完整Q&A轮追加写入 `问答记录.md`
...
**\question 随机提问（AI向用户提问）**：

⚠️ **重要约束**：\question 是"AI向用户提出问题并等待回答"的纯交互流程，不是生成内容。严禁AI自问自答，严禁在用户回答前写入任何文件。

**对话输出格式**（必须在对话中完整展示，不走文件优先，因为这是纯对话互动）：

标题：`## 🎲 随机提问（第N轮）`

正文按以下结构组织（每轮聚焦一个不同的命盘要点，避免重复）：

1. **切入点**（1-2句引入）：从命盘中最有故事性的要点入手——格局的特殊组合、四化的交互影响、大限的关键转折、来因宫的前世今生、双子星的内外反差等
2. **命盘证据**（引用具体宫位和星曜）：用引用的方式展示命盘中支撑该问题的星曜、宫位组合。需包含星曜亮度、四化状态、宫位位置
3. **深层解读**（分析这些证据的含义）：解释为什么要问这个、这个组合为什么特别、有什么矛盾和张力
4. **倪师断语引用**（如果有相关）：如果倪师对该星曜/宫位组合有经典断语，必须引用
5. **提问**：以 `👉` 开头，问一个开放式的、跟用户真实生活经验相关的问题，让用户愿意回答

**⚠️ 禁止行为**：
- ❌ 自问自答（AI提出问题后立即自己回答）
- ❌ 在用户回答前写入问答记录.md
- ❌ 把问题简化为一两行（必须按上述5段结构展开）
- ❌ 连续两轮重复同一个命盘要点
- ❌ 在问题中暗示"正确答案"
...
4. 完整Q&A轮（问题+用户回答+AI分析）增量追加写入 `问答记录.md`，同时重新运行 `md2html.js` 生成 `问答记录.html`

**格式示例**（不可省略任何段落）：
```
## 🎲 随机提问（第三轮）

这次我想聊一个你命盘里极有意思的内外反差——

> 你命宫在寅：紫微旺+天府庙，帝相并临，天生稳重、有主见、有领导范儿。
>
> 但你迁移宫在申：七杀庙旺对冲命宫——你一出门，整个世界就变成了一座竞技场。

这意味着什么？在家里、在熟悉的环境里，你是帝王和丞相；但一旦踏入社会、进入竞争环境，你面对的就是七杀——杀破狼之首，主肃杀、竞争、刀光剑影。

更妙的是，七杀（申）、破军（子·夫妻宫）、贪狼（辰·福德宫）三颗星构成了申子辰水局三合——等于说你的迁移、婚姻、精神世界，被一个隐藏的杀破狼三角绑在了一起。

倪师讲过一句很有意思的话："七杀坐迁移的人，走到哪里都有人跟他对着干，但七杀庙旺就是能在竞争中胜出。"

👉 你有没有注意到，你这一生有两个完全不同的"模式"？在安逸时稳重，在竞争中好胜——这两种模式切换时，你有没有过"两个自己在打架"的感觉？
```
...
| `\question` | 随机提问（AI向用户提问） | `\switch` | 切换体系 |
...
**⚠️ 文件优先**：如果回答超过20行，必须追加写入 `ziwei-output/{日期_HHmm}_{出生信息}/问答记录.md`，对话中只输出简要回答（见"文件优先输出策略"）。`\question` 指令的特殊规则：提问阶段纯对话输出（不写文件，按5段格式完整提问），等用户回答后再将Q&A轮追加写入问答记录.md。
