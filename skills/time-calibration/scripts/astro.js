...
function usage() {
  console.log('用法: node scripts/astro.js "YYYY-M-D" hourIndex gender [outputDir] [--year YYYY]');
  console.log('');
  console.log('参数:');
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--year' && i + 1 < args.length) {
      result.year = parseInt(args[i + 1], 10);
      i++;
    } else {
  ...
  result.solarDate = positional[0];
  result.hourIndex = parseInt(positional[1], 10);
  result.gender = positional[2];
  result.outputDir = positional[3] || null;
