...
- **盘外验证体系全面升级**：从"2-3条断语"升级为"5-7条多维度断语"
  - 新增3类断语类型：事实断语（强验证力）、倾向断语（中验证力）、事件断语（强验证力）
  - 新增5维度覆盖要求：家庭背景/身体特征/人际模式/财运特征/关键年份，必须覆盖≥3个
  - 新增断语评分机制：完全吻合+2/部分吻合+1/不吻合-1/无法判断0
  - 新增次优时辰盘外验证：置信度为低或中时，必须对次优时辰也做盘外验证对比
  - 新增降级规则：≥2条不吻合或事实断语全部未命中→降级
  - SKILL.md 第4轮输出模板升级为表格化断语列表+盘外验证得分+次优时辰对比

## [3.0.2] - 2026-05-27

### Added

- **真太阳时换算**（`scripts/solar-time.js`）：基于出生地经度的真太阳时校正，含中国城市经度数据库（34省级行政区），来自 `sources/ziwei-doushu/lib/ziwei/share.ts` 的 `calcTrueSolarBranch` 函数
- **出生地收集**：反推时辰第1轮新增出生地为必须收集项，用于真太阳时换算
- **真太阳时换算步骤**：时段选择后必须运行 `solar-time.js` 换算，候选时辰基于真太阳时而非时钟时辰

### Changed

- **默认使用真太阳时**：排盘以真太阳时为准（原"倪师不用真太阳时"已改为默认使用）
- `SKILL.md`：新增真太阳时换算命令文档、第1轮新增出生地收集+真太阳时换算步骤、参考文件索引新增 `solar-time.js`
- `calibration-protocol.md`：出生地点从可选改为必须收集、新增 2.4 真太阳时换算步骤、4.2 真太阳时规则从"默认不使用"改为"默认使用"、常见陷阱新增"忽略真太阳时换算"

## [3.0.1] - 2026-05-27

### Changed

- **第1轮时段选择优化**：新增固定6档时段多选步骤，替代模糊的"大致时间范围"问法
- **时段选择必须展示完整时辰对照表**：用户选择前先看到13时辰→时间范围→对应时段的完整表格
- **所有轮次输出模板固定化**：第1-4轮的输出格式改为"固定输出格式"，强调必须原样输出
- **第2轮对比表细化**：列头含时间范围，身宫/化禄/化忌格式细化，性格描述必须含做事风格/社交/情绪三维度
- **第3轮事件验证表细化**：列头含时间范围，每事件后必须输出累计得分
- **第4轮结论细化**：验证过程必须列出所有候选时辰及每个被排除时辰的排除原因
- 同步更新 `calibration-protocol.md` 时段选择规则
